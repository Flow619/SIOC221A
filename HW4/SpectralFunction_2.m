function [Mean_P,freq,Error_Bars] = SpectralFunction_2(x,dt,Num_windows)
...
% Deg_Freedom = (size(X_chunks,1)*size(X_chunks,2))/length(x);
...
for i = 1:((2*Num_windows)-1)
    ...
    [FFT_chunks(:,i),freq] = MySpectrum(X_chunks(:,i).',dt,"OFF"); % 4.c FFT each segment
end
...
%% Normalize
Mean_P = Mean_P/ mean(hann(Num_per_Seg).^2);

%% Error Bars
nu = Num_windows;
error_high = nu/(chi2inv(0.025,nu));
error_low = nu/(chi2inv(0.975,nu));
Error_Bars = [error_low,error_high];
